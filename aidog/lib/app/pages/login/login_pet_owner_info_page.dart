...
class _LoginPetOwnerInfoPageState extends State<LoginPetOwnerInfoPage> {
            ...
            child: InkWell(
              onTap: () {
                ...
                setState(() {
                  ...
                  // AALog(
                  //     "删除的图片:${Provider.of<LoginProvider>(context, listen: false).photoArr.length}");
                });
              },
              child: Image.asset(
                width: ScreenAdapter.width(32),
  ...
  void openPhoto() async {
    ...
    setState(() {
      ...
      // Provider.of<LoginProvider>(context, listen: false).selectPhotoAssets =
      //     _selectPhotoAssets;
      // AALog(
      //     "选择的图片结果:${Provider.of<LoginProvider>(context, listen: false).selectPhotoAssets}");

      AALog("选择的图片结果:$_selectPhotoAssets");

      ///上传宠主 宠物多张图片
      // await uploadMutableImage(_selectPhotoAssets, (arr) {
      //   Provider.of<LoginProvider>(context, listen: false).photoArr = arr;
      //   setState(() {});
      // });
      // List arr = await LoginApi.uploadImages(_selectPhotoAssets);
      // AALog("arrarrarrarr $arr");
    });
  }
  ...
  void petComplete() async {
    // AALog(
    //     "宠主完成界面${Provider.of<LoginProvider>(context, listen: false).photoArr}");
    //登录跳转到tabbar首页
    ...
    // Provider.of<LoginProvider>(context, listen: false)
    //     .uploadMutableImage(_selectPhotoAssets);
    ...
    // AALog("返回的map:$data");
    // _photoArr.add(data);
    // if (_photoArr.length == _selectPhotoAssets.length) {
    //   AALog("最终图片数组:$_photoArr");

    // String photoarr = jsonEncode(_photoArr);
    // LoginApi.loginAddDog(
    //   Provider.of<LoginProvider>(context, listen: false).addressStr,
    //   _petDate,
    //   gender,
    //   _selectPetZhaogufangshi.toString(),
    //   petNameController.text,
    //   // photoarr,
    //   _selectPetKuangquanyimiaoState.toString(),
    //   onSuccess: (data) {
    //     AALog("data $data");
    //     //登录跳转到tabbar首页
    //     NavigationUtil.getInstance().pushAndRemoveUtil(
  ...
  // Future<void> uploadMutableImage(
  //     List<AssetEntity> selectPhotos, PhotoArrUpdateCallback onUpdate) async {
  //   List arr = [];
  ...
  //         // Provider.of<LoginProvider>(context, listen: false).photoArr.add(data);
  //         arr.add(data);
  //         // AALog(
  //         //     "最终图片数组:${Provider.of<LoginProvider>(context, listen: false).photoArr}");
  //         AALog("最终图片数组:$arr");
  //         onUpdate(arr);
  //       });
  //     },
  ...
  Future<String?> getImagePath(originFile) async {
    ...
    // var newFile = await DuCompress.image(originFile);
    // if (newFile != null) {
    //   final AssetEntity? asset = await PhotoManager.editor.saveImage(originFile.readAsBytesSync(), filename: filename)
    //  }
  }
